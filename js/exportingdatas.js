...
(function (Highcharts) {
    ...
    // Utility function to show a popup with data table and export buttons
    function showPopup(dataTableHTML, downloadCSV, downloadXLS) {
        // Create and display the popup
        const popupHTML = `
            <div id="highcharts-popup" style="
                position: fixed; top: 10%; left: 10%; width: 80%; height: 80%;
                background: white; border: 1px solid #ccc; box-shadow: 0 4px 8px rgba(0,0,0,0.2);
                overflow: auto; z-index: 10000; padding: 20px; box-sizing: border-box;
            ">
                <button id="highcharts-popup-close" style="
                    position: absolute; top: 10px; right: 10px; padding: 10px;
                    background: #f44336; color: white; border: none; cursor: pointer;
                ">Close</button>
                <button id="highcharts-popup-download-csv" style="
                    margin: 10px; padding: 10px; background: #4CAF50; color: white;
                    border: none; cursor: pointer;
                ">Download CSV</button>
                <button id="highcharts-popup-download-xls" style="
                    margin: 10px; padding: 10px; background: #2196F3; color: white;
                    border: none; cursor: pointer;
                ">Download XLS</button>
                <div id="highcharts-popup-content">
                    ${dataTableHTML}
                </div>
            </div>
        `;
        ...
        document.getElementById('highcharts-popup-close').addEventListener('click', function () {
            document.getElementById('highcharts-popup').remove();
        });

        document.getElementById('highcharts-popup-download-csv').addEventListener('click', downloadCSV);
        document.getElementById('highcharts-popup-download-xls').addEventListener('click', downloadXLS);
    }

    // Define the export data module
    Highcharts.addEvent(Highcharts.Chart, 'render', function () {
        ...
        function downloadCSV() {
            ...
            a.href = url;
            a.download = chart.getFilename() + '.csv';
            a.click();
            URL.revokeObjectURL(url);
        }

        // Function to export as XLS
        function downloadXLS() {
            const tableHTML = chart.getTable();
            const blob = new Blob([tableHTML], { type: 'application/vnd.ms-excel' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = chart.getFilename() + '.xls';
            a.click();
            URL.revokeObjectURL(url);
        }

        // Function to display data table in a popup
        function viewData() {
            const tableAST = chart.getTableAST();
            const tableHTML = new Highcharts.Renderer().tableHTML(tableAST); // Assuming a method to convert AST to HTML
            showPopup(tableHTML, downloadCSV, downloadXLS);
        }

        // Add a button to the chart to open the data table in a popup
        if (!chart.exportDivElements) {
            chart.exportDivElements = [];
            const button = chart.renderer.button('View Data', 0, 0, viewData)
                .add();
            chart.exportDivElements.push(button);
        }
    });

    // Extend Highcharts.Chart to include methods for exporting data
    Highcharts.Chart.prototype.getCSV = function () {
        return this.getDataRows().map(row => row.join(',')).join('\n');
    };

    Highcharts.Chart.prototype.getTable = function () {
        return '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>Sheet1</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--><meta charset=UTF-8></head><body>' + this.getTableHTML() + '</body></html>';
    };

    Highcharts.Chart.prototype.getTableAST = function () {
        return this.getTableAST();
    };

    // Get data rows for CSV export
    Highcharts.Chart.prototype.getDataRows = function () {
        return this.series.map(series => {
            return series.data.map(point => [point.x, point.y]);
        });
    };
